import Icon from "./Icon";

// Path data from @fortawesome/free-solid-svg-icons' faCircleArrowRight,
// extracted directly rather than re-sourced, to guarantee an exact match to
// what was already rendering.
const PATH =
  "M0 256a256 256 0 1 0 512 0A256 256 0 1 0 0 256zM297 385c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l71-71L120 280c-13.3 0-24-10.7-24-24s10.7-24 24-24l214.1 0-71-71c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0L409 239c9.4 9.4 9.4 24.6 0 33.9L297 385z";

export default function ArrowRightCircleIcon({
  className,
}: {
  className?: string;
}) {
  return <Icon viewBox="0 0 512 512" path={PATH} className={className} />;
}
